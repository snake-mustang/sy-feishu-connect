...
已经安装过旧版本？先在旧机器人窗口按 `Ctrl+C` 停止运行，再执行：
...
你的 `~/.sy-feishu-connect/config.toml` 会继续保留，App ID、App Secret、中文姓名和飞书工号都不用重新填写，也不要再次运行 `setup`。
...
GitHub 安装链接本身不会提供“谁下载了”的明细；真正统计发生在用户完成首次配置和后续使用时。公司版已自动接入统一统计，普通用户不需要配置统计地址。
