...
`im:message.group_msg` 是敏感权限。如果只让群聊 @ 机器人时触发，可以删掉这一行再导入。
...
| 机器人自定义菜单事件 | `application.bot.menu_v6` | 接收底部菜单点击 |

`application.bot.menu_v6` 必须添加。少了它，底部菜单点了不会触发。
...
4. 菜单事件 `application.bot.menu_v6` 添加了吗？
5. 事件订阅方式是不是长连接？
6. 群聊里有没有 @机器人？
...
1. 事件里有没有订阅 `application.bot.menu_v6`。
2. 改完菜单后有没有发布新版本。
